...
class Bench:
    def __init__(self, ctx):
        ...
        try:
            self.connect_options = {
                'client_keys': [self.manager.settings.instance_key_path],
                'connect_timeout': 30,
    ...
    def _generate_config(self, hosts, node_parameters, bench_parameters):
        ...
        cmd = CommandMaker.generate_bls_keys(len(hosts), bench_parameters.bls_threshold, PathMaker.bls_file_default_path()).split()
            ...
            for burst in bench_parameters.burst:
                ...
                for i in range(bench_parameters.runs):
                    ...
                    try:
                        ...
                        branch_name = self.settings.branch
                        dt = datetime.now()
                        date = dt.date()
                        time = dt.time()
                        log_path = PathMaker.logs_path()
                        cmd= r'mkdir -p backup_logs/'+str(date) + str("-") + str(branch_name) + str("-") + str(burst) + str("-") +str(time) + ' ; mv ' + log_path + '/*.* ' + 'backup_logs/'+str(date) + str("-") + str(branch_name) + str("-") + str(burst) + str("-") +str(time) +'/'
                        subprocess.run([cmd], shell=True, stderr=subprocess.DEVNULL)
