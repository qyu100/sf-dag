...
class Bench:
    ...
    def _backup_logs(
        ...
        if not exists(log_path):
            return
        log_files = [name for name in listdir(log_path) if isfile(join(log_path, name))]
        if not log_files:
            return
        ...
        backup_dir = join(
            'blockup_logs',
            f'{timestamp}-{branch_name}-{nodes}nodes-burst{burst}-run{run}-rate{rate}-tx{tx_size}',
        )
        ...
        metadata = {
            ...
            'run_started_at': run_started_at.isoformat(timespec='microseconds'),
            'run_finished_at': run_finished_at.isoformat(timespec='microseconds'),
            'run_elapsed_seconds': f'{(run_finished_at - run_started_at).total_seconds():.3f}',
        }
        ...
        for name in log_files:
            src = join(log_path, name)
            move(src, join(backup_dir, name))
            ...
            for burst in bench_parameters.burst:
                ...
                for i in range(bench_parameters.runs):
                    ...
                    try:
                        ...
                        await self._download_logs(consensus_only, committee=committee_copy)
    ...
    def run(self, bench_parameters_dict, node_parameters_dict, debug=False, consensus_only=True, update=True):
        ...
        if 'tx_size' not in node_parameters_dict and 'tx_size' in bench_parameters_dict:
            node_parameters_dict['tx_size'] = bench_parameters_dict['tx_size']
        try:
