...
class BlsKey:
    def __init__(self, nameg2, nameg1, secret):
        self.nameg2 = nameg2
        self.nameg1 = nameg1
        self.secret = secret
    ...
    def from_file(cls, filename):
        ...
        with open(filename, 'r') as f:
            data = load(f)
        return cls(data['nameg2'], data['nameg1'], data['secret'])
...
class BenchParameters:
    def __init__(self, json):
        try:
            ...
            # if not burst:
            #     raise ConfigError('Missing input burst')
            # self.burst = [int(x) for x in burst]
            self.bls_threshold = int(json['bls_threshold'])
