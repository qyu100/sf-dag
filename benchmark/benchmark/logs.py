...
class LogParser:
    def __init__(self, clients, primaries, workers, burst, faults=0, consensus_only=False):
    ...
    def result(self):
        ...
        write_to_csv(round(consensus_tps), round(consensus_bps), round(
    ...
    def process(cls, directory, burst, faults=0, consensus_only=False):
        ...
        return cls(clients, primaries, workers, burst, faults=faults, consensus_only=consensus_only)


def write_to_csv(consensus_tps, consensus_bps, consensus_latency, e2e_tps, e2e_bps, e2e_latency, blps, burst, header_size, csv_file_path):
    # Open the CSV file in append mode
    with open(csv_file_path, mode='a', newline='') as csv_file:
        writer = csv.writer(csv_file)
        column_names = ['Consensus TPS', 'Consensus BPS', 'Consensus Latency',
                        'E2E TPS', 'E2E BPS', 'E2E Latency', 'BLPS', 'Burst', 'Header size']
        # If the file is empty, write the header
        if csv_file.tell() == 0:
            writer.writerow(column_names)

        # Write the extracted data to the CSV file
        writer.writerow([consensus_tps, consensus_bps, consensus_latency,
                        e2e_tps, e2e_bps, e2e_latency, blps, burst, header_size])
